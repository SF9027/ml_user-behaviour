...
由于数据过大，我们决定随机抽取1000个数据，创建一个新的数据集来进行后续工作。

```{r}
userdataset <- sample_n(userdataset_orig, 1000, replace = FALSE)
```
...
userdataset1 <- userdataset1 %>% select(-1)
...
userdataset2 <- merge(userdataset1, gender, by = "gender" )
