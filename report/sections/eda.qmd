...
我们把数据表中的数据分为了2大类，numeric和factor。我们将"country","civilityGenderId","hasAnyApp","hasProfilePicture","langEn","langFr","langDe","langEs","langIt","genderFamale","genderMale"这些列的attribute从numeric改为factor。剩余的列的attribute从integer变为numeric。
...
The goal of our project is to build the model to predit whether users are latent on this platform. So here we do a comparison of users with latent statu and non-latent statu.
...
```
从图中可以看出，数据结构极度不平衡。但造成该图的结果，可能是由于我们随机筛选造成的，对此，我们会尝试使用分组筛选。
